...
BinaryTree* BINARYEXPRESSIONTREE_insertOperator(BinaryTree* firstTree,
    ...
    binaryTree->valueChar = operatorEx;
    binaryTree->left = firstTree;
    binaryTree->right = secondTree;
    return binaryTree;
}

void evalExpressionRecursive(BinaryTree* binaryTree, DStack* dStack){
    if(binaryTree->left) evalExpressionRecursive(binaryTree->left, dStack);
    if(binaryTree->right) evalExpressionRecursive(binaryTree->right, dStack);
    
    if(48 <= binaryTree->valueChar && binaryTree->valueChar <= 57)
        dStackPush(dStack, binaryTree->valueChar-48);
    else{
        double value1=0, value2=0;
        value1 = dStackPop(dStack);
        value2 = dStackPop(dStack);
        switch(binaryTree->valueChar){
        case '+':
...
double BINARYEXPRESSIONTREE_evalExpression(BinaryTree* binaryTree){
    ...
    DStack* dStack = dStackCreateStack();
    evalExpressionRecursive(binaryTree, dStack);
    double result = dStackPop(dStack);
    
    dStack = dStackFree(dStack);
    return result;
}
