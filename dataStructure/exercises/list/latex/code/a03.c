 ...
 */
NodeCharFreq* listOfCharFreq(char* literal,int sizeLiteral,
                                        NodeCharFreq* first){
    ...
    if(literal[0] != 0){
        ...
        if((sizeLiteral-totalFreq) > 0){
            ...
            char* newCharLiteral = malloc(sizeof(sizeLiteral-
                                                    totalFreq));
            
            fillNewChar(literal+1,newCharLiteral, literal);
            
            NodeCharFreq* newList = listOfCharFreq(newCharLiteral,
                                    sizeLiteral-totalFreq,first);
            free(newCharLiteral);
            return newList;
        }
        else
